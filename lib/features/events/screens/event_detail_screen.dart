...
class _EventDetailScreenState extends ConsumerState<EventDetailScreen> {
  ...
  Future<void> _handleJoinEvent(String uid) async {
      ...
      final result = await showDialog<String>(
        ...
        builder: (dialogContext) => AlertDialog(
          ...
          content: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text('Get Verified to Join',
                style: TheyDiTextStyles.headlineMedium,
                textAlign: TextAlign.center),
            const SizedBox(height: 10),
            Text(
              'Verified profiles build trust with hosts and other attendees.\nYou can still join without verifying.',
    ...
    // Already verified — join directly
    _joinEvent(uid);
  }
  ...
  Future<void> _joinEvent(String uid) async {
    ...
    try {
      ...
      if (isFree && !isHostApproval) {
        ...
        await eventRef.update({
          'pendingUids': FieldValue.arrayUnion([uid])
        });
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text(
                  'Request sent! You\'ll be notified to pay after approval 📩'),
              backgroundColor: Colors.green));
        }
      }
    } catch (e) {
  ...
  Future<void> _cancelRequest(String uid) async {
    final ok = await _confirmCancel(
        ...
        message:
            'Are you sure you want to cancel your join request for "${_event.title}"?');
    if (!ok) return;
    setState(() => _isProcessing = true);
    try {
      await FirebaseFirestore.instance
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Request cancelled'), backgroundColor: Colors.grey));
      }
    } catch (e) {
      if (mounted) {
        AppErrorUtils.showErrorSnackBar(context, e);
        ...
        actions: [
          ...
          TextButton(
            ...
            child: Text('Yes, Cancel',
                style:
                    TheyDiTextStyles.labelMedium.copyWith(color: Colors.red)),
          ),
        ],
      ),
    );
    ...
    switch (state) {
      case _BookingState.none:
        if (isFull) {
          ...
          label = 'Join Now — Free';
        } else if (_approvalType == 'Host Approval') {
          label = 'Request to Join';
        } else {
          label = 'Join for ₹${_event.price.toInt()}';
        }
    ...
    if (isOutlined) {
      return SizedBox(
        ...
        child: OutlinedButton(
          ...
          style: OutlinedButton.styleFrom(
            side: BorderSide(
                color: outlineColor ?? TheyDiColors.divider, width: 1.5),
            shape:
          ...
          child: _isProcessing
              ...
              : Text(label,
                  style: TheyDiTextStyles.labelLarge.copyWith(
                      color: outlineColor ?? TheyDiColors.textSecondary)),
        ),
      );
    }
...
class _InfoCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
            ...
            border: Border.all(color: TheyDiColors.divider)),
        child: Row(children: [
          Icon(icon, color: TheyDiColors.primary, size: 18),
          const SizedBox(width: 8),
          Expanded(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
      ...
      Container(
        ...
        child: Column(children: [
          _DetailRow(
              ...
              label: 'Capacity',
              value: '$currentAttendees / $maxAttendees attending'),
          if (genderBalance.isNotEmpty) ...[
            const SizedBox(height: 12),
            _DetailRow(
                icon: Icons.people_outline,
            ...
            if (genderBalance == 'Ratio' && genderRatio != null) ...[
              ...
              Padding(
                  ...
                  child: Column(children: [
                    ...
                    Row(children: [
                      _RatioLabel(
                          'Male ${(genderRatio!['male'] ?? 50).toInt()}%',
                          Colors.blue),
                      const SizedBox(width: 10),
                      _RatioLabel(
                          'Female ${(genderRatio!['female'] ?? 25).toInt()}%',
                          Colors.pink),
                      const SizedBox(width: 10),
                      _RatioLabel(
                          'Other ${(genderRatio!['other'] ?? 25).toInt()}%',
                          Colors.green),
                    ]),
                  ])),
            ],
          ...
          if (approvalType.isNotEmpty) ...[
            const SizedBox(height: 12),
            _DetailRow(
                icon: approvalType == 'Host Approval'
                    ? Icons.verified_user_outlined
                    : Icons.flash_on_outlined,
                label: 'Entry',
                value: approvalType)
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        const SizedBox(width: 8),
        Text('Safety & Trust', style: TheyDiTextStyles.displayLarge)
      ]),
      const SizedBox(height: 12),
      Container(
        padding: const EdgeInsets.all(16),
        ...
        child: Column(children: [
          ...
          const _SafetyItem(
              ...
              text: 'Community guidelines enforced',
              isActive: true),
          const SizedBox(height: 10),
          const _SafetyItem(
...
class _SafetyItem extends StatelessWidget {
  ...
  final bool isActive;
  const _SafetyItem(
      {required this.icon, required this.text, required this.isActive});
  @override
  Widget build(BuildContext context) => Row(children: [
        Icon(icon,
            size: 16, color: isActive ? Colors.green : TheyDiColors.textMuted),
        const SizedBox(width: 10),
        Text(text,
