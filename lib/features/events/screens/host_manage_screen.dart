...
class _HostManageScreenState extends ConsumerState<HostManageScreen> {
  ...
  Future<void> _checkExistingCircle() async {
    ...
    if (mounted) setState(() { _existingCircle = circle; _checkingCircle = false; });
  }
  ...
  Future<void> _cancelEvent(EventModel event) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('This will notify all ${event.currentAttendees} attendees. This cannot be undone.',
            style: TheyDiTextStyles.bodyMedium.copyWith(color: TheyDiColors.textSecondary)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false),
              child: Text('Go Back', style: TheyDiTextStyles.labelMedium.copyWith(color: TheyDiColors.textSecondary))),
          TextButton(onPressed: () => Navigator.pop(ctx, true),
              child: Text('Cancel Event', style: TheyDiTextStyles.labelMedium.copyWith(color: Colors.red))),
        ],
      ),
    );
  ...
  Future<void> _createEventCircle(EventModel event) async {
    ...
    if (event.attendeeUids.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('No approved attendees yet'), backgroundColor: Colors.orange));
      return;
    }
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: TheyDiColors.card,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: Text('Create Event Circle?', style: TheyDiTextStyles.headlineMedium),
        content: Text('This will create a group chat called "${event.title} Circle" with all ${event.currentAttendees} approved attendees.',
            style: TheyDiTextStyles.bodyMedium.copyWith(color: TheyDiColors.textSecondary)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false),
              child: Text('Cancel', style: TheyDiTextStyles.labelMedium.copyWith(color: TheyDiColors.textSecondary))),
          TextButton(onPressed: () => Navigator.pop(ctx, true),
              child: Text('Create', style: TheyDiTextStyles.labelMedium.copyWith(color: TheyDiColors.primary))),
        ],
      ),
    );
    if (confirmed != true) return;
    ...
    try {
      ...
      for (final uid in event.attendeeUids) {
        ...
        } catch (_) { attendeeNames.add('Member'); }
      }
      final circle = await EventCircleService.createEventCircle(
          event: event, attendeeUids: event.attendeeUids, attendeeNames: attendeeNames);
      if (mounted) {
        setState(() => _existingCircle = circle);
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('"${circle.name}" created! 🎉'), backgroundColor: Colors.green));
        ...
        child: SafeArea(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Padding(
              ...
              child: Row(children: [
                IconButton(icon: const Icon(Icons.arrow_back, color: TheyDiColors.textPrimary), onPressed: () => context.pop()),
              ...
              child: StreamBuilder<DocumentSnapshot>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (!snapshot.hasData || !snapshot.data!.exists) {
                    return Center(child: Text('Event not found', style: TheyDiTextStyles.bodySmall));
                  }
                  ...
                  return ListView(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(event.title, style: TheyDiTextStyles.headlineMedium),
                          const SizedBox(height: 8),
                          Wrap(spacing: 8, runSpacing: 6, children: [
                            _StatPill(icon: Icons.people, label: '${event.currentAttendees}/${event.maxAttendees} attending', color: Colors.green),
                            _StatPill(icon: Icons.hourglass_top, label: '${event.pendingCount} pending', color: Colors.amber),
                            if (!event.isFree && event.approvedPendingPaymentUids.isNotEmpty)
                              _StatPill(icon: Icons.payment_outlined, label: '${event.approvedPendingPaymentUids.length} awaiting payment', color: Colors.blue),
                          ]),
                          if (event.currentAttendees >= 2) ...[
                            const SizedBox(height: 16),
                            _checkingCircle
                                ? const Center(child: SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: TheyDiColors.primary)))
                                : SizedBox(
                                    width: double.infinity,
                                    child: _existingCircle != null
                                        ? OutlinedButton.icon(
                                            onPressed: () => context.push(AppRoutes.circleChat, extra: _existingCircle!),
                                            icon: const Icon(Icons.chat_bubble_outline, color: TheyDiColors.primary, size: 18),
                                            label: Text('Open Circle — ${_existingCircle!.name}',
                                        ...
                                        : DecoratedBox(
                                            ...
                                            child: ElevatedButton.icon(
                                              onPressed: _isProcessing ? null : () => _createEventCircle(event),
                                              icon: _isProcessing
                                                  ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                                                  : const Icon(Icons.group_add_outlined, color: Colors.white, size: 18),
                                              label: Text(_isProcessing ? 'Creating...' : 'Create Event Circle',
                          ...
                          SizedBox(
                            ...
                            child: OutlinedButton.icon(
                              ...
                              label: const Text('Cancel Event', style: TextStyle(color: Colors.red, fontWeight: FontWeight.w600)),
                              style: OutlinedButton.styleFrom(
                      ...
                      // ── Pending Requests ──
                      Text('Pending requests', style: TheyDiTextStyles.labelLarge).animate(delay: 150.ms).fade(duration: 300.ms),
                      ...
                      if (event.pendingUids.isEmpty)
                        ...
                        ...List.generate(event.pendingUids.length, (index) {
                          ...
                          ).animate(delay: Duration(milliseconds: 200 + 50 * index)).fade(duration: 300.ms).slideY(begin: 0.1, end: 0);
                        }),

                      // ── Awaiting Payment ──
                      if (!event.isFree && event.approvedPendingPaymentUids.isNotEmpty) ...[
                        const SizedBox(height: 24),
                        Text('Awaiting payment', style: TheyDiTextStyles.labelLarge).animate(delay: 250.ms).fade(duration: 300.ms),
                        const SizedBox(height: 4),
                        ...
                        ...event.approvedPendingPaymentUids.map((uid) =>
                            _AwaitingPaymentCard(userUid: uid, onViewProfile: () => _viewProfile(context, uid))
                      ...
                      // ── Approved Attendees ──
                      Text('Approved attendees', style: TheyDiTextStyles.labelLarge).animate(delay: 300.ms).fade(duration: 300.ms),
                      const SizedBox(height: 12),
                      ...
                      if (event.attendeeUids.isEmpty)
                        ...
                        ...List.generate(event.attendeeUids.length, (index) {
                          ...
                          return _AttendeeCard(userUid: uid, isHostUid: event.creatorUid, onViewProfile: () => _viewProfile(context, uid))
                              .animate(delay: Duration(milliseconds: 350 + 40 * index)).fade(duration: 300.ms);
                        }),
  ...
  Widget _buildEmptyPending() {
    return Container(
      ...
      child: Center(child: Column(children: [
        ...
        const SizedBox(height: 4),
        Text('All caught up!', style: TheyDiTextStyles.caption.copyWith(color: TheyDiColors.textSecondary)),
      ])),
    );
  }
}
...
class _StatPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      Text(label, style: TheyDiTextStyles.caption.copyWith(color: color, fontWeight: FontWeight.w600)),
    ]),
  );
}
...
class _PendingRequestCardState extends State<_PendingRequestCard> {
  ...
  static Color _tagColor(String tag) {
    switch (tag) {
      ...
      case 'Age Mismatch':    return Colors.orange;
      case 'Gender Overflow': return Colors.blue;
      case 'Age + Gender':    return Colors.red;
  ...
  static IconData _tagIcon(String tag) {
    switch (tag) {
      case 'Perfect':         return Icons.check_circle_outline;
      case 'Age Mismatch':    return Icons.cake_outlined;
      case 'Gender Overflow': return Icons.people_outline;
      case 'Age + Gender':    return Icons.warning_amber_rounded;
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  if (tag == 'Age Mismatch' || tag == 'Age + Gender') ...[
                    ...
                    Text('— outside age group',
                        style: TheyDiTextStyles.caption.copyWith(color: tagColor.withValues(alpha: 0.8), fontSize: 10)),
                  ],
        ...
        return Container(
          ...
          child: Row(children: [
            ...
            Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                const Icon(Icons.hourglass_top_outlined, size: 11, color: Colors.blue), const SizedBox(width: 4),
                Text('Awaiting payment', style: TheyDiTextStyles.caption.copyWith(color: Colors.blue, fontSize: 10, fontWeight: FontWeight.w600)),
              ]),
            ),
          ]),
        );
...
class _AttendeeCard extends StatefulWidget {
  final String userUid; final String isHostUid; final VoidCallback onViewProfile;
  const _AttendeeCard({required this.userUid, required this.isHostUid, required this.onViewProfile});
  @override
...
class _AttendeeCardState extends State<_AttendeeCard> {
  ...
  Future<void> _loadStatus() async {
    ...
    final friendDoc = await db.collection('users').doc(myUid).collection('friends').doc(widget.userUid).get();
    if (friendDoc.exists) { if (mounted) setState(() { _status = FriendStatus.friends; _loadingStatus = false; }); return; }
    final sentSnap = await db.collection('users').doc(widget.userUid).collection('friendRequests')
        .where('fromUid', isEqualTo: myUid).where('status', isEqualTo: 'pending').get();
    if (sentSnap.docs.isNotEmpty) { if (mounted) setState(() { _status = FriendStatus.requestSent; _loadingStatus = false; }); return; }
    final receivedSnap = await db.collection('users').doc(myUid).collection('friendRequests')
        .where('fromUid', isEqualTo: widget.userUid).where('status', isEqualTo: 'pending').get();
    if (receivedSnap.docs.isNotEmpty) { if (mounted) setState(() { _status = FriendStatus.requestReceived; _loadingStatus = false; }); return; }
    if (mounted) setState(() { _status = FriendStatus.none; _loadingStatus = false; });
  }

  Future<void> _sendRequest(String toName) async {
    setState(() => _sending = true);
    await FriendsService.sendFriendRequest(toUid: widget.userUid, toName: toName);
    if (mounted) {
      setState(() { _status = FriendStatus.requestSent; _sending = false; });
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<DocumentSnapshot>(
      future: FirebaseFirestore.instance.collection('users').doc(widget.userUid).get(),
      builder: (context, snapshot) {
        final data = snapshot.data?.data() as Map<String, dynamic>? ?? {};
        final name = data['displayName'] ?? 'Loading...';
        return Container(
          ...
          child: Row(children: [
            MouseRegion(cursor: isMe ? SystemMouseCursors.basic : SystemMouseCursors.click,
              child: GestureDetector(onTap: isMe ? null : widget.onViewProfile, child: Row(mainAxisSize: MainAxisSize.min, children: [
                Container(width: 36, height: 36, decoration: BoxDecoration(gradient: TheyDiColors.gradientPrimary, borderRadius: BorderRadius.circular(10)),
                    child: Center(child: Text(name.isNotEmpty ? name[0].toUpperCase() : '?',
                        style: TheyDiTextStyles.labelLarge.copyWith(color: Colors.white)))),
                const SizedBox(width: 10),
                Row(children: [
                  Text(name, style: TheyDiTextStyles.labelMedium),
                  if (!isMe) ...[const SizedBox(width: 4), Icon(Icons.open_in_new, size: 11, color: TheyDiColors.textMuted)],
                ]),
              ]))),
            const Spacer(),
            isHostMember
                ? Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                    decoration: BoxDecoration(color: TheyDiColors.primary.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(8)),
                    child: Text('Admin', style: TheyDiTextStyles.caption.copyWith(color: TheyDiColors.primary, fontWeight: FontWeight.w600)))
                : Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                    decoration: BoxDecoration(color: Colors.green.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(8)),
                    child: Text('Approved', style: TheyDiTextStyles.caption.copyWith(color: Colors.green, fontWeight: FontWeight.w600))),
            if (!isMe) ...[
              const SizedBox(width: 8),
              _loadingStatus
                  ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: TheyDiColors.primary))
    ...
    switch (_status) {
      case FriendStatus.friends:
        return Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
            decoration: BoxDecoration(color: Colors.green.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(8)),
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              const Icon(Icons.check, size: 12, color: Colors.green), const SizedBox(width: 4),
              Text('Connected', style: TheyDiTextStyles.caption.copyWith(color: Colors.green, fontWeight: FontWeight.w600, fontSize: 10)),
            ]));
      case FriendStatus.requestSent:
        return Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
            decoration: BoxDecoration(color: TheyDiColors.card, borderRadius: BorderRadius.circular(8), border: Border.all(color: TheyDiColors.divider)),
            child: Text('Requested', style: TheyDiTextStyles.caption.copyWith(color: TheyDiColors.textSecondary, fontSize: 10)));
      default:
        return GestureDetector(onTap: _sending ? null : () => _sendRequest(name),
          child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
              decoration: BoxDecoration(gradient: TheyDiColors.gradientPrimary, borderRadius: BorderRadius.circular(8)),
              child: _sending
                  ? const SizedBox(width: 12, height: 12, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                  : Text('Connect', style: TheyDiTextStyles.caption.copyWith(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 10))));
    }
  }
}
