...
// FIX: App bar Row overflow on narrow mobile screens — title now uses
//      Flexible so it truncates instead of pushing Share/Edit buttons off-screen.
// ─────────────────────────────────────────────────────────────────────────────
...
class _CommunityInfoScreenState extends State<CommunityInfoScreen> {
  ...
  Future<void> _pickGroupPhoto() async {
    ...
    try {
      ...
      final url = await CloudflareUpload.uploadBytes(bytes, '${_community.id}.jpg');
      if (url == null) throw Exception('Cloudflare upload failed');
  ...
  Future<void> _removeMember(String uid, String name) async {
    final confirmed = await _confirmDialog(
      title: 'Remove $name?',
      body: 'They will be removed from this community and notified.',
      confirm: 'Remove',
    ...
    await CommunityService.removeMemberFromCommunity(
        communityId: _community.id, userUid: uid, userName: name);
    await _refresh();

    if (mounted) {
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('$name removed'), backgroundColor: Colors.grey));
    }
  }

  Future<void> _leaveGroup() async {
    final myName = _community.memberMap[_myUid] ?? 'You';
    final confirmed = await _confirmDialog(
      title: 'Leave Community?',
      body: 'Are you sure you want to leave "${_community.name}"?',
      confirm: 'Leave',
      confirmColor: Colors.red,
    );
    if (!confirmed) return;
    ...
    await CommunityService.removeMemberFromCommunity(
        communityId: _community.id, userUid: _myUid, userName: myName);

    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('You left the community'), backgroundColor: Colors.grey),
      );
      context.pop();
      context.pop();
    }
  ...
  Future<void> _deleteGroup() async {
    final confirmed = await _confirmDialog(
      ...
      body: 'This will permanently delete "${_community.name}" and all its messages.',
      confirm: 'Delete',
      confirmColor: Colors.red,
    );
    if (!confirmed) return;
  ...
  Future<void> _clearChat() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Chat cleared from your view'), backgroundColor: Colors.orange),
      );
    }
  }
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  Expanded(
                    child: Text('Report "${_community.name}"',
              ...
              Text('Select a reason for reporting',
                  style:
                      TheyDiTextStyles.bodySmall.copyWith(color: TheyDiColors.textSecondary)),
              const SizedBox(height: 16),
              ..._kCommunityReportReasons.map((reason) => GestureDetector(
                      ...
                      child: Row(
                        children: [
                          Icon(
                            ...
                            size: 18,
                            color: selectedReason == reason ? Colors.red : TheyDiColors.textMuted,
                          ),
                          const SizedBox(width: 12),
                          Expanded(
                            child: Text(reason,
              ...
              SizedBox(
                ...
                child: DecoratedBox(
                  decoration: BoxDecoration(
                    color: selectedReason != null ? Colors.red : TheyDiColors.divider,
                    borderRadius: BorderRadius.circular(12),
                  ),
                  child: ElevatedButton(
                    onPressed: selectedReason == null
                        ...
                        : () async {
                            Navigator.pop(ctx);
                            await _submitCommunityReport(selectedReason!);
                          },
  ...
  Future<void> _submitCommunityReport(String reason) async {
    await FirebaseFirestore.instance.collection('reports').add({
      'reportedCommunityId': _community.id,
      'reportedCommunityName': _community.name,
      'reporterUid': _myUid,
      'reason': reason,
      'type': 'community',
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Report submitted. Thank you for helping keep TheyDi safe.'),
  ...
  Future<void> _blockCommunity() async {
    final confirmed = await _confirmDialog(
      title: 'Block Community?',
      body: 'You will leave "${_community.name}" and it will be hidden from you. This cannot be undone.',
      confirm: 'Block',
      confirmColor: Colors.red,
    );
    if (!confirmed) return;

    final myName = _community.memberMap[_myUid] ?? 'User';
    await CommunityService.removeMemberFromCommunity(
        communityId: _community.id, userUid: _myUid, userName: myName);

    await FirebaseFirestore.instance.collection('users').doc(_myUid).update({
      'blockedCommunityIds': FieldValue.arrayUnion([_community.id]),
    });

    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('"${_community.name}" blocked'), backgroundColor: Colors.red),
      );
      context.pop();
      context.pop();
    }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(10),
          border: Border.all(color: TheyDiColors.primary.withValues(alpha: 0.35), width: 1),
        ),
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
  ...
  Widget build(BuildContext context) {
    final members = _community.memberMap;
    final uniqueCount = members.length;
    final initial = _community.initials;
      ...
      body: Container(
        ...
        child: SafeArea(
          child: Column(
            children: [
              // ── App Bar ──────────────────────────────────────────────────────
              // FIX: title wrapped in Flexible so it truncates on narrow
              // screens instead of pushing Share/Edit/Cancel off-screen.
              Padding(
                padding: const EdgeInsets.fromLTRB(4, 8, 8, 0),
                child: Row(
                  children: [
                    // Back button — fixed width, never shrinks
                    IconButton(
                  ...
                  children: [
                              ...
                              Positioned(
                                ...
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(
                                    ...
                                    border:
                                        Border.all(color: const Color(0xFF0D0D14), width: 2),
                                  ),
                    ...
                    _editing
                        ? TextField(
                            ...
                            style: TheyDiTextStyles.displayMedium.copyWith(fontSize: 20),
                    ...
                    const SizedBox(height: 6),
                    Center(
                      child: Text('$uniqueCount member${uniqueCount == 1 ? '' : 's'}',
                    ...
                    if (!_isHost)
                      SizedBox(
                        ...
                        child: OutlinedButton.icon(
                          ...
                          icon: const Icon(Icons.exit_to_app, color: Colors.orange, size: 18),
                    ...
                    _CommunityActionTile(
                      ...
                      subtitle: 'Leave and hide this community permanently',
                      color: Colors.red,
                      onTap: _blockCommunity,
                    ),
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(label, style: TheyDiTextStyles.labelMedium.copyWith(color: color)),
                  Text(subtitle,
                      style: TheyDiTextStyles.caption.copyWith(color: TheyDiColors.textMuted)),
                ],
              ),
...
class _AddMembersSheet extends StatefulWidget {
  ...
  const _AddMembersSheet({required this.community, required this.onMembersAdded});
...
class _AddMembersSheetState extends State<_AddMembersSheet> {
  ...
  Future<void> _loadCandidates() async {
    ...
    final currentMemberUids = widget.community.memberUids.toSet();
  ...
  Future<void> _addSelected() async {
    ...
    for (final uid in _selectedUids) {
      ...
      await FirebaseFirestore.instance
        ...
        'body': '$myName added you to "${widget.community.name}"',
        'type': 'social',
        'isRead': false,
        'createdAt': Timestamp.now(),
      });
            ...
            child: Container(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  hintStyle:
                      TheyDiTextStyles.bodySmall.copyWith(color: TheyDiColors.textMuted),
                  prefixIcon: Icon(Icons.search, color: TheyDiColors.textMuted, size: 20),
                ...
                : _filtered.isEmpty
                    ...
                    : ListView(
                        ...
                        children: _filtered.map((candidate) {
                          final isSelected = _selectedUids.contains(candidate['uid']);
                          ...
                          return GestureDetector(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                borderRadius: BorderRadius.circular(12),
                                border: Border.all(
                                  color: isSelected ? TheyDiColors.primary : TheyDiColors.divider,
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        if (city.isNotEmpty || username.isNotEmpty)
                                          Text(city.isNotEmpty ? city : username,
                                              style: TheyDiTextStyles.caption),
                                      ],
                                    ),
                                  ),
