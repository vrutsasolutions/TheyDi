...
import '../../circles/models/message_model.dart';
...
final _communitySharedPlayer = ap.AudioPlayer();
...
class _CommunityChatScreenState extends ConsumerState<CommunityChatScreen>
  ...
  void initState() {
    ...
    FirebaseFirestore.instance
      ...
      final updated = CommunityModel.fromFirestore(doc);
...
// still sends fine, it just falls back to a placeholder icon in the bubble.
  ...
  Future<void> _sendMessage({
    ...
    try {
      ...
      final encryptedText = await EncryptionService.encrypt(text, _community.id);
  ...
  Future<void> _startRecording() async {
    ...
    try {
      if (kIsWeb) {
        ...
        final path =
            '$tmpDir/voice_community_${DateTime.now().millisecondsSinceEpoch}.aac';
        await _recorder.start(
  ...
  void _openInfo() => context.push(AppRoutes.communityInfo, extra: _community);
  ...
  Future<void> _confirmAndSendMedia(
    ...
    // This mirrors the DM chat screen's flow so gallery/camera photos get
    // the same edit-before-send experience in communities.
    if (type == 'image') {
                      ...
                      child: Row(children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            image: _community.coverImageUrl != null &&
                                    _community.coverImageUrl!.isNotEmpty
                                ? DecorationImage(
                                    image:
                                        NetworkImage(_community.coverImageUrl!),
                                    fit: BoxFit.cover,
                                  )
                                : null,
                            gradient: (_community.coverImageUrl == null ||
                                    _community.coverImageUrl!.isEmpty)
                        ...
                        Expanded(
                            child: Column(
                              ...
                              Text(_community.name,
                                  ...
                                  overflow: TextOverflow.ellipsis),
                              Text(
                                  '${_community.memberUids.length} members · Tap for info',
              ...
              builder: (context, snapshot) {
                ...
                if (docs.isEmpty) {
                  return Center(
                    ...
                    const SizedBox(height: 12),
                    Text('Start the conversation in ${_community.name}!',
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Column(children: [
                      ...
                      else if (msgType == 'community_invite')
                        _CommunityInviteBubble(
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'Message ${_community.name}...',
...
class _CommunityVoiceBubble extends StatefulWidget {
  ...
  const _CommunityVoiceBubble(
      ...
      required this.seen});
  @override
  State<_CommunityVoiceBubble> createState() => _CommunityVoiceBubbleState();
}
...
class _CommunityImageBubble extends StatelessWidget {
  final String imageUrl;
  final bool isMine;
  final String senderName;
  final String timeLabel;
  ...
  @override
  Widget build(BuildContext context) {
    final heroTag = 'community_image_${imageUrl}_$timeLabel';
    return Padding(
...
class _CommunityInviteBubble extends StatelessWidget {
...
class _CommunityEventShareBubble extends StatelessWidget {
  ...
  const _CommunityEventShareBubble({
...
class _CommunityProfileShareBubble extends StatelessWidget {
  ...
  const _CommunityProfileShareBubble({
...
class _CommunityVideoBubble extends StatelessWidget {
...
class _CommunityDocumentBubble extends StatelessWidget {
  ...
  const _CommunityDocumentBubble({
...
class _CommunityVoiceBubbleState extends State<_CommunityVoiceBubble> {
  ...
  void initState() {
    super.initState();
    _stateSub = _communitySharedPlayer.onPlayerStateChanged.listen((s) {
      if (!mounted) return;
    ...
    _posSub = _communitySharedPlayer.onPositionChanged.listen((pos) {
  ...
  Future<void> _togglePlay() async {
    if (_isPlaying) {
      await _communitySharedPlayer.pause();
      setState(() => _isPlaying = false);
    } else {
      await _communitySharedPlayer.stop();
      await _communitySharedPlayer.play(ap.UrlSource(widget.audioUrl));
